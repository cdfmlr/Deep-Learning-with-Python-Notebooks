...
token_text = keras.preprocessing.text.text_to_word_sequence(text, split=' ')
...
callbacks_list = [
    ...
    keras.callbacks.ReduceLROnPlateau(
        ...
        patience=2,            # monitor 在 patience 轮内没有改善，则触发降低学习率
    ),
    # 不再改善时中断训练
    keras.callbacks.EarlyStopping(
        monitor='loss',           # 要验证的指标
        patience=4,             # 如果 monitor 在多于 patience 轮内（比如这里就是10+1=11轮）没有改善，则中断训练
    ),
]
...
for temperature in [0.2, 0.5, 1.0, 1.2]:
    ...
    for i in range(100):    # 生成 400 个 token
        # 编码当前文本
        # text_cut = jieba.cut(generated_text)
        text_cut = keras.preprocessing.text.text_to_word_sequence(generated_text)
        sampled = []
        for i in text_cut:
